...
// Поэлементное деление на скаляр
...
// Генерация единичного вектора
function gen_ed(n, pos: integer): Tmatr_arr;

// Генерация единичного вектора-строки
function gen_ed_row(n, pos: integer): Tmatr_arr;

// Умножение матрицы на скаляр
function multi_calc(matr: Tmatr_arr; scalar: real): Tmatr_arr;

function gmethod(const A: Tmatr_arr; out ko: integer): Tmatr_arr;

procedure scalar_product_via_matrix(v: Tmatr_arr; out square: real;
  out len: real);

procedure print_angle_degrees_between_vectors(a, b: Tmatr_arr);
...
// begin
// raise Exception.Create('Leading element is zero or near-zero at row ' +
// IntToStr(i + 1) + '. Matrix is singular.');
// end;
//
// factor := T[i, i];
// for k := 0 to n - 1 do
...
// begin
// T[i, k] := T[i, k] / factor;
// if (factor <> 0.0) and (factor <> 1.0) and (factor <> -1.0) then
// begin
// inc(ko);
// end;
// end;
...
// begin
// var
// mult_result := factor * T[i, k];
// T[j, k] := T[j, k] - mult_result;
// end;
//
// for k := n to 2 * n - 1 do
// begin
...
begin
  ...
  begin
    if Abs(T[i, i]) < 1E-10 then
      raise Exception.Create('Leading element is zero at row ' +
        IntToStr(i + 1));

    for i1 := i + 1 to n - 1 do
    begin
      if Abs(T[i1, i]) < 1E-10 then
        continue;
  ...
  begin
    ...
    begin
      ...
      for j1 := j + 1 to n + j - 1 do
      begin
        T[i, j1] := T[i, j1] - k * T[j, j1];
      end;
      inc(ko, n - 1);
...
// Функция scalar_product_via_matrix: вычисляет скалярное произведение через C^T * C.
// a, b: векторы (Tmatr_arr размером n x 1)
// out square: выходной параметр - квадрат скалярного произведения (a·b)^2
// out len: выходной параметр - длина (норма) скалярного произведения |a·b|
procedure scalar_product_via_matrix(v: Tmatr_arr; out square: real;
  out len: real);
var
  i, j, n: integer;
begin
  n := length(v);
  if (n = 0) then
    raise Exception.Create('Vector must be non-empty');

//  if (length(v[0]) <> 1) then
//    raise Exception.Create('Input must be a column vector (n x 1)');

  square := 0.0;
  for i := 0 to n - 1 do
  begin
    square := square + v[i, 0] * v[i, 0];
  end;
...
// Функция print_angle_degrees_between_vectors: вычисляет и выводит угол в градусах между двумя векторами.
// a, b: векторы (Tmatr_arr размером n x 1)
procedure print_angle_degrees_between_vectors(a, b: Tmatr_arr);
var
  n, i: integer;
  dot_product, norm_a, norm_b, cos_theta, theta_rad, theta_deg: real;
begin

  n := length(a);
  if (n <> length(b)) or (n = 0) then
    raise Exception.Create('Vectors must have the same length and be non-empty');

  if (length(a[0]) <> 1) or (length(b[0]) <> 1) then
    raise Exception.Create('Inputs must be column vectors (n x 1)');

  // Вычисляем скалярное произведение a·b
  dot_product := 0.0;
  for i := 0 to n - 1 do
    dot_product := dot_product + a[i, 0] * b[i, 0];

  // Вычисляем норму ||a||
  norm_a := 0.0;
  for i := 0 to n - 1 do
    norm_a := norm_a + a[i, 0] * a[i, 0];
  norm_a := Sqrt(norm_a);

  // Вычисляем норму ||b||
  norm_b := 0.0;
  for i := 0 to n - 1 do
    norm_b := norm_b + b[i, 0] * b[i, 0];
  norm_b := Sqrt(norm_b);

  // Проверка на деление на ноль
  if (norm_a < 1E-10) or (norm_b < 1E-10) then
    raise Exception.Create('One or both vectors have zero length.');

  cos_theta := dot_product / (norm_a * norm_b);

  // Ограничиваем cos_theta диапазоном [-1, 1] из-за погрешностей вычислений
  if cos_theta > 1.0 then
    cos_theta := 1.0
  else if cos_theta < -1.0 then
    cos_theta := -1.0;

  writeln('Cos between vectors: ', cos_theta);

  theta_rad := ArcCos(cos_theta); // Угол в радианах
  theta_deg := theta_rad * 180.0 / Pi; // Преобразуем в градусы

  writeln('Angle between vectors: ', theta_deg:0:6, ' degrees.');
end;
